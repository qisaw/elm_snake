...
Elm.Main.make = function (_elm) {
   ...
   var _op = {};
   var isSnakeAlive = function (_p0) {
      var _p1 = _p0;
      var _p4 = _p1.segments;
      return _U.eq($Set.size($Set.fromList(A2($List.map,function (_p2) {    var _p3 = _p2;return {ctor: "_Tuple2",_0: _p3.x,_1: _p3.y};},_p4))),
      $List.length(_p4));
   };
   var None = {ctor: "None"};
   var timeActionSignal = A2($Signal.map,$Basics.always(None),$Time.fps(8));
   var Right = {ctor: "Right"};
   var Left = {ctor: "Left"};
   var Down = {ctor: "Down"};
   var Up = {ctor: "Up"};
   var createAction = function (m) {    return _U.eq(m.x,1) ? Right : _U.eq(m.x,-1) ? Left : _U.eq(m.y,1) ? Up : _U.eq(m.y,-1) ? Down : None;};
   var actionSignal = A2($Signal.map,createAction,$Keyboard.arrows);
   var allSignal = A2($Signal.merge,actionSignal,timeActionSignal);
   var padding = 2;
   var tileSize = 20;
   var initSingleCoOrd = {x: tileSize / 2,y: 0};
   var initCoOrds = function (end) {
      return A2($List.map,function (i) {    return {x: (0 - i) * tileSize + initSingleCoOrd.x,y: initSingleCoOrd.y};},_U.range(0,end));
   };
   var snake0 = {direction: None,segments: initCoOrds(20)};
   var game0 = {snake: snake0,finished: false};
   var createNewSegment = F2(function (c,a) {
      var _p5 = c;
      if (_p5.ctor === "Nothing") {
            return initSingleCoOrd;
         } else {
            var _p7 = _p5._0;
            var _p6 = a;
            switch (_p6.ctor)
            {case "Up": return _U.update(_p7,{y: _p7.y + tileSize});
               case "Down": return _U.update(_p7,{y: _p7.y - tileSize});
               case "Left": return _U.update(_p7,{x: _p7.x - tileSize});
               case "Right": return _U.update(_p7,{x: _p7.x + tileSize});
               default: return _p7;}
         }
   });
   var getSnakeSegments = F2(function (list,action) {
      return A2($List._op["::"],A2(createNewSegment,$List.head(list),action),A2($List.take,$List.length(list) - 1,list));
   });
   var update = F2(function (a,g) {
      if (g.finished) return g; else {
            var s = g.snake;
            var s$ = function () {
               var _p8 = a;
               switch (_p8.ctor)
               {case "Up": return _U.update(s,{direction: Up});
                  case "Down": return _U.update(s,{direction: Down});
                  case "Left": return _U.update(s,{direction: Left});
                  case "Right": return _U.update(s,{direction: Right});
                  default: var _p9 = s.direction;
                    if (_p9.ctor === "None") {
                          return s;
                       } else {
                          return _U.update(s,{segments: A2(getSnakeSegments,s.segments,_p9)});
                       }}
            }();
            return _U.update(g,{snake: s$,finished: $Basics.not(isSnakeAlive(s$))});
         }
   });
   var gameSignal = A3($Signal.foldp,update,game0,allSignal);
   var createSnakeSegment = function (c) {
      return A2($Graphics$Collage.move,
      {ctor: "_Tuple2",_0: c.x,_1: c.y},
      A2($Graphics$Collage.filled,$Color.black,$Graphics$Collage.square(tileSize - padding)));
   };
   var createSnake = function (s) {    return A2($List.map,createSnakeSegment,s.segments);};
   var drawGame = F2(function (g,_p10) {
      var _p11 = _p10;
      var _p13 = _p11._0;
      var _p12 = _p11._1;
      var floatH = $Basics.toFloat(_p12);
      var floatW = $Basics.toFloat(_p13);
      return g.finished ? $Graphics$Element.show("GAME OVER") : A3($Graphics$Collage.collage,
      _p13,
      _p12,
      A2($List._op["::"],
      A2($Graphics$Collage.filled,$Color.lightGrey,A2($Graphics$Collage.rect,floatW,floatH)),
      A2($List._op["::"],A2($Graphics$Collage.outlined,$Graphics$Collage.solid($Color.grey),A2($Graphics$Collage.rect,floatW,floatH)),createSnake(g.snake))));
   });
   var main = A3($Signal.map2,drawGame,gameSignal,$Window.dimensions);
   var Directions = F2(function (a,b) {    return {direction: a,list: b};});
   var KeyboardInput = F2(function (a,b) {    return {x: a,y: b};});
   var Snake = F2(function (a,b) {    return {direction: a,segments: b};});
   var Game = F2(function (a,b) {    return {snake: a,finished: b};});
   var CoOrd = F2(function (a,b) {    return {x: a,y: b};});
   var BoundingSquare = F4(function (a,b,c,d) {    return {tLeft: a,tRight: b,bLeft: c,bRight: d};});
   return _elm.Main.values = {_op: _op
                             ...
                             ,Right: Right
                             ,None: None
                             ,initSingleCoOrd: initSingleCoOrd
                             ...
                             ,update: update
                             ,createSnakeSegment: createSnakeSegment
                             ,createSnake: createSnake
                             ,drawGame: drawGame
                             ,timeActionSignal: timeActionSignal
